...
Raw_Socket::Raw_Socket(uint16_t recipient_port, uint32_t recipient_IP, const std::string& data)
    : recipient_port(recipient_port), recipient_IP(recipient_IP), data(data), raw_socket(INVALID_SOCKET),
...
void Raw_Socket::init()
{
    ...
    int optval = 1;
    if (setsockopt(raw_socket, IPPROTO_IP, IP_HDRINCL, reinterpret_cast<char*>(&optval), sizeof(optval)) == SOCKET_ERROR) {
...
void Raw_Socket::send_packet()
{
    ...
    if (sendto(raw_socket, send_packet.data(), size, 0, reinterpret_cast<struct sockaddr*>(&info), sizeof(info)) == SOCKET_ERROR) {
